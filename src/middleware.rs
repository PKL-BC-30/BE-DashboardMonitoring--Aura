    ...
    S::Future: 'static,
{
    ...
    fn call(&self, request: ServiceRequest) -> Self::Future {
        ...
        if path == "/users/register" || path == "/login" || path == "/users/delete"
        || path == "/verify_otp" || path == "/forgot_password" || path == "/users/{id}"
        || path == "/users/update"|| path == "/user"|| path == "/chart/goldar"
        || path == "/chart/gender" || path == "/chart/umur" || path == "/chart/job" 
        || path == "/logout/{user_id}"{
